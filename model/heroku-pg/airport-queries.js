...
async function editInfo(text,date,callback){
    ...
    const sql1 = `insert into "modifies" ("username","info_ID","modification_date") VALUES('user8',(select "info_ID" from "General_info" where "title"='${text.titlos}'),'${date}')`;
    try{
        const client = await connect();
        const res = await client.query(sql);
        await client.release()
        const client1 = await connect();
        const res1 = await client1.query(sql1);
