...
async function getRoutes(airlineName,date_c,airportName,isDest,id1,callback){
    ...
    if(airlineName!='-' && airportName!='-' && id1!=''){
        ...
        var sql = ` Select "airline_name","flight_ID" ,"city","flight_date"::text,"expected_time","terminal","gate_number","gate_name",CASE WHEN "expected_time"<='${cur_time}' and "flight_date"='${full_date}' THEN 1 ELSE 0 END AS "left" from "flies" natural join "Airport" join "Airline" on "flies"."airline_ID" = "Airline"."airline_ID" join "Gate" on "Airline".gate_code = "Gate"."gate_ID"  WHERE "flight_date"='${date_c}' and "is_destination"=${isDest} and "Airline"."airline_name"='${airlineName}' order by "expected_time"`;
    }else if(airlineName!='-' && airportName=='-' && id1!=''){
        var sql = ` Select "airline_name","flight_ID" ,"city","flight_date"::text,"expected_time","terminal","gate_number","gate_name",CASE WHEN "expected_time"<='${cur_time}' and "flight_date"='${full_date}' THEN 1 ELSE 0 END AS "left" from "flies" natural join "Airport" join "Airline" on "flies"."airline_ID" = "Airline"."airline_ID" join "Gate" on "Airline".gate_code = "Gate"."gate_ID"  WHERE "flight_date"='${date_c}' and "is_destination"=${isDest} and "Airport"."airline_name"='${airlineName}' and "flies"."flight_ID"='${id1}' order by "expected_time"`;
    }
    try{
        const client = await connect();
        const res = await client.query(sql);
